	...
	tt := map[string]struct {
		...
		"add a file": {
			...
			replicaClient: func(mockCtrl *gomock.Controller) server {
				...
				s.EXPECT().PatchFile(gomock.Any(), &pb.PatchFileRequest{FullPath: "created_file", FullContents: "bar"}).Return(nil, nil).Times(2)
				return s
			},
		},
		"create a dir": {
			...
			replicaClient: func(mockCtrl *gomock.Controller) server {
				s := mock_watcher.NewMockserver(mockCtrl)
				s.EXPECT().CreateDir(gomock.Any(), &pb.CreateDirRequest{FullPath: "created_dir"}).Return(nil, nil)
				return s
			},
		},
		"remove a dir": {
		...
		"1 new file, 1 new dir, 1 new file in dir, update first file, remove both files": {
			...
			replicaClient: func(mockCtrl *gomock.Controller) server {
				s := mock_watcher.NewMockserver(mockCtrl)
				s.EXPECT().PatchFile(gomock.Any(), gomock.Any()).Return(nil, nil).AnyTimes()
				s.EXPECT().CreateDir(gomock.Any(), gomock.Any()).Return(nil, nil)
		...
		"problem during rm file": {
			...
			replicaClient: func(mockCtrl *gomock.Controller) server {
				s := mock_watcher.NewMockserver(mockCtrl)
				s.EXPECT().PatchFile(gomock.Any(), gomock.Any()).Return(nil, nil).Times(2)
		...
		"problem during file patch": {
			...
			replicaClient: func(mockCtrl *gomock.Controller) server {
				s := mock_watcher.NewMockserver(mockCtrl)
				s.EXPECT().PatchFile(gomock.Any(), gomock.Any()).Return(nil, fmt.Errorf("server patch error")).AnyTimes()
				return s
			},
			err: "context cancelled",
		},
		"problem during dir creation": {
			...
			replicaClient: func(mockCtrl *gomock.Controller) server {
				s := mock_watcher.NewMockserver(mockCtrl)
				s.EXPECT().CreateDir(gomock.Any(), gomock.Any()).Return(nil, fmt.Errorf("server create dir error"))
				return s
			},
			err: "context cancelled",
		},
